...
MODEL_FOLDER = 'models'
HAYSTACK_CLASSES = {0: 'Non-protest', 1: 'Protest'}
FORM_CLASSES     = {0: 'Blockade/slowdown/disruption', 1: 'Boycott',
					2: 'Hunger strike', 3: 'March', 4: 'Non-protest',
					5: 'Rally/demonstration', 6: 'Riot',
					7: 'Strike/walkout/lockout'
				}
ISSUE_CLASSES    = {0: 'Anti-colonial/political independence',
					1: 'Anti-war/peace', 2: 'Criminal justice system',
					3: 'Democratisation', 4: 'Economy/inequality',
					5: 'Environmental', 6: 'Foreign policy',
					7: 'Human and civil rights', 8: 'Labour & work',
					9: 'Non-protest', 10: 'Political corruption/malfeasance',
					11: 'Racial/ethnic rights', 12: 'Religion',
					13: 'Social services & welfare', 14: 'None of the above'
				}
TARGET_CLASSES   = {0: 'Domestic government', 1: 'Foreign government',
					2: 'Individual', 3: 'Intergovernmental organisation',
					4: 'Non-protest', 5: 'Private/business'
				}
TASK_DICT        = {'form': FORM_CLASSES,
					'issue': ISSUE_CLASSES,
					'target': TARGET_CLASSES
				}
TASK_LENGTHS     = {'form': 8, 'issue': 15, 'target': 6}
	...
	if args.task == 'haystack':
		if torch.cuda.is_available():
			model = RobertaForSequenceClassification.from_pretrained(
				f'{MODEL_FOLDER}/haystack/cuda/',
				config=config
			)
		else:
			model = RobertaForSequenceClassification.from_pretrained(
				f'{MODEL_FOLDER}/haystack/cpu/',
				config=config
			)
		...
		if torch.cuda.is_available():
			model = RobertaProtestAuxClassification.from_pretrained(
				f'{MODEL_FOLDER}/{args.task}/cuda/',
				config=config
			)
		else:
			model = RobertaProtestAuxClassification.from_pretrained(
				f'{MODEL_FOLDER}/{args.task}/cpu/',
				config=config
			)
		...
		if torch.cuda.is_available():
			...
			model = RobertaMultiTaskClassification.from_pretrained(
				f'{MODEL_FOLDER}/multi/cpu/',
				config=config
			)

	if torch.cuda.is_available():
		os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(str(id) \
			for id in args.gpu_devices)
		model.cuda()
	else:
		model.cpu()

	# All tokenizers are the same (for Roberta)
...
def predict(model, inputs, args):
	...
	if args.mc_samples > 0:
		...
		if args.task in ['form', 'issue', 'target']:
			aux_std_err = [aux_mat[:, i].std() \
				for i in range(TASK_LENGTHS[args.task])]
			...
			form_std_err = [form_mat[:, i].std() \
				for i in range(TASK_LENGTHS['form'])]
			...
			issue_std_err = [issue_mat[:, i].std() \
				for i in range(TASK_LENGTHS['issue'])]
			...
			target_std_err = [target_mat[:, i].std() \
				for i in range(TASK_LENGTHS['target'])]
...
def print_results(results, args):
	...
	if args.mc_samples > 0:
		print(f"Haystack mean probabilities: " + \
			f"{np.round(results['haystack_mean'], 6)}")
		print(f"Haystack standard errors: " + \
			f"{np.round(results['haystack_std_err'], 6)}")
		print(f"Haystack entropy: {np.round(results['haystack_entropy'], 6)}")
		pprint(HAYSTACK_CLASSES)

	print(f"{'=' * 40}")
	if args.task in ['form', 'issue', 'target']:
		print(f"{args.task.title()} prediction: " + \
			f"{results[f'{args.task}_prediction']}")
		if args.mc_samples > 0:
			print(f"{args.task.title()} mean probabilities: " + \
				f"{np.round(results[f'{args.task}_mean'], 6)}")
			print(f"{args.task.title()} standard errors: " + \
				f"{np.round(results[f'{args.task}_std_err'], 6)}")
			print(f"{args.task.title()} entropy: " + \
				f"{np.round(results[f'{args.task}_entropy'], 6)}")
		...
		if args.mc_samples > 0:
			print(f"Form mean probabilities: " + \
				f"{np.round(results['form_mean'], 6)}")
			print(f"Form standard errors: " + \
				f"{np.round(results['form_std_err'], 6)}")
		...
		if args.mc_samples > 0:
			print(f"Issue mean probabilities: " + \
				f"{np.round(results['issue_mean'], 6)}")
			print(f"Issue standard errors: " + \
				f"{np.round(results['issue_std_err'], 6)}")
		...
		if args.mc_samples > 0:
			print(f"Target mean probabilities: " + \
				f"{np.round(results['target_mean'], 6)}")
			print(f"Target standard errors: " + \
				f"{np.round(results['target_std_err'], 6)}")
...
def store_results(results, args):
	...
	with open(f"{output_path}{out_file}", 'w+') as f:
		...
		if args.mc_samples > 0:
			f.write(f"Haystack mean probabilities: " + \
				f"{np.round(results['haystack_mean'], 6)}\n")
			f.write(f"Haystack standard errors: " + \
				f"{np.round(results['haystack_std_err'], 6)}\n")
			f.write(f"Haystack entropy: " + \
				f"{np.round(results['haystack_entropy'], 6)}\n")
			f.write(f"{pformat(HAYSTACK_CLASSES, indent=4)}\n")

		f.write(f"{'=' * 40}\n")
		if args.task in ['form', 'issue', 'target']:
			f.write(f"{args.task.title()} prediction: " + \
				f"{results[f'{args.task}_prediction']}\n")
			if args.mc_samples > 0:
				f.write(f"{args.task.title()} mean probabilities: " + \
					f"{np.round(results[f'{args.task}_mean'], 6)}\n")
				f.write(f"{args.task.title()} standard errors: " + \
					f"{np.round(results[f'{args.task}_std_err'], 6)}\n")
				f.write(f"{args.task.title()} entropy: " + \
					f"{np.round(results[f'{args.task}_entropy'], 6)}\n")
			...
			if args.mc_samples > 0:
				f.write(f"Form mean probabilities: " + \
					f"{np.round(results['form_mean'], 6)}\n")
				f.write(f"Form standard errors: " + \
					f"{np.round(results['form_std_err'], 6)}\n")
				f.write(f"Form entropy: " + \
					f"{np.round(results['form_entropy'], 6)}\n")
				f.write(f"{pformat(FORM_CLASSES, indent=4)}\n")
			f.write(f"{'=' * 40}\n")

			f.write(f"Issue prediction: {results['issue_prediction']}\n")
			if args.mc_samples > 0:
				f.write(f"Issue mean probabilities: " + \
					f"{np.round(results['issue_mean'], 6)}\n")
				f.write(f"Issue standard errors: " + \
					f"{np.round(results['issue_std_err'], 6)}\n")
				f.write(f"Issue entropy: " + \
					f"{np.round(results['issue_entropy'], 6)}\n")
				f.write(f"{pformat(ISSUE_CLASSES, indent=4)}\n")
			f.write(f"{'=' * 40}\n")

			f.write(f"Target prediction: {results['target_prediction']}\n")
			if args.mc_samples > 0:
				f.write(f"Target mean probabilities: " + \
					f"{np.round(results['target_mean'], 6)}\n")
				f.write(f"Target standard errors: " + \
					f"{np.round(results['target_std_err'], 6)}\n")
				f.write(f"Target entropy: " + \
					f"{np.round(results['target_entropy'], 6)}\n")
				f.write(f"{pformat(ISSUE_CLASSES, indent=4)}\n")
			f.write(f"{'=' * 40}\n")
