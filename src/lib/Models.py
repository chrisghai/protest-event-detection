...
class RobertaProtestAuxClassification(DistilBertPreTrainedModel):
	...
	def forward(
		...
		aux_inputs = torch.cat((self.dropout(transformer_out), label_logits),
			dim=-1)
		...
		if labels is not None:
			...
			label_loss = loss_fct(label_logits.view(-1, self.num_labels),
				labels.long().view(-1))
			aux_loss = loss_fct(aux_logits.view(-1, self.aux_classes),
				aux.long().view(-1))
...
class RobertaMultiTaskClassification(DistilBertPreTrainedModel):
	...
	def __init__(self, config):
		...
		self.form_enc = nn.Linear(config.hidden_size + self.num_labels,
			config.hidden_size)
		self.form_clf = nn.Linear(config.hidden_size, self.form_classes)

		self.issue_enc = nn.Linear(config.hidden_size + self.num_labels,
			config.hidden_size)
		self.issue_clf = nn.Linear(config.hidden_size, self.issue_classes)

		self.target_enc = nn.Linear(config.hidden_size + self.num_labels,
			config.hidden_size)
